...
import { Client, RpcTransactionRequest, createPublicClient, createWalletClient, custom, http } from "viem";
import { baseGoerli } from "viem/chains";
import { WalletClientSigner, type SmartAccountSigner, PublicErc4337Client, createPublicErc4337Client } from "@alchemy/aa-core";
import { AlchemyProvider } from "@alchemy/aa-alchemy";
import { LightSmartContractAccount, getDefaultLightAccountFactory } from "@alchemy/aa-accounts";
import { BASE_GOERLI_ALCHEMY_RPC_URL, BASE_GOERLI_ENTRYPOINT_ADDRESS, BASE_GOERLI_PAYMASTER_URL } from "../lib/constants";
...
export const SmartAccountProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const bundler: PublicErc4337Client = useMemo(
    () =>
      createPublicErc4337Client({
        chain: baseGoerli,
        rpcUrl: `${BASE_GOERLI_ALCHEMY_RPC_URL}/${process.env.NEXT_PUBLIC_ALCHEMY_API_KEY!}`,
      }),
    [],
  );
  ...
  const sendSponsoredUserOperation = async (transactionRequest: RpcTransactionRequest) => {
    ...
    const userOpHash = await bundler.sendUserOperation(signedUserOp, BASE_GOERLI_ENTRYPOINT_ADDRESS);
    return userOpHash;
  };
